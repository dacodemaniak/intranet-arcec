<?php
/**
 * @name paramBase.class.php : Gestion des paramètres de base de l'intranet ARCEC
...
class paramBase extends \wp\formManager\admin{
	...
	public function __construct(){
		...
		$this->mapper = new \arcec\Mapper\paramdefinitionMapper();
	...
	protected function set(){
		...
		$field->setId("frmCode")
		->setName($this->mapper->getTableName() . "." . $this->mapper->getColumnPrefix() . "code")
		->setLabel("Codification")
		->setCss("control-label",true)
		->setCss("col-sm-5",true)
		->isRequired()
		->setCss("form-control")
		->isDisabled($disabledCode)
		->setValue($this->mapper->getObject()->code);
		...
		$parentTable = $this->mapper->getSchemeDetail("type_table_param_id","parent_table");
		$factory = new \wp\Patterns\factory($this->mapper->getNameSpace() . $parentTable . "Mapper");
		
		$field->setId("frmTypeParam")
			->setName($this->mapper->getTableName() . ".type_table_param_id")
			->setLabel("Type de paramètre")
			->setCss("control-label",true)
			->setCss("col-sm-5",true)
			->setCss("form-control")
			->isRequired()
			->setMapping($factory->addInstance(),array("value" => "id", "content"=>"type"))
			->setValue($this->mapper->getObject()->type_table_param_id)
		;
